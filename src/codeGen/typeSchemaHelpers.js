...
function fieldType(type, name, value, useInputs) {
  ...
  if (typeof value === "object" && value.__isDate) {
    ...
    switch (value) {
      ...
      case LongArrayType:
        return `[Long${ofNonNullBang}]${nonNullBang}`;
      case FloatArrayType:
...
function fieldMutations(k, fields) {
  ...
  if (typeof value === "object" && value.__isDate) {
    ...
    if (value === BoolType) {
      ...
      return [`${k}: Long`, `${k}_INC: Long`, `${k}_DEC: Long`];
    } else if (value === "Float") {
    ...
    } else if (value === LongArrayType) {
      return [
        `${k}: [Long]`,
        `${k}_PUSH: Long`,
        `${k}_CONCAT: [Long]`,
        `${k}_UPDATE: LongArrayUpdate`,
        `${k}_UPDATES: [LongArrayUpdate]`,
        `${k}_PULL: [Long]`,
        `${k}_ADDTOSET: [Long]`
      ];
...
function queriesForField(objectToCreate, fieldName, realFieldType) {
  ...
  switch (realFieldType) {
    ...
    case FloatArrayType:
      let singleType;
      if (realFieldType === IntArrayType) {
        singleType = 'Int';
      } else if (realFieldType === LongArrayType) {
        singleType = 'Long';
      } else {
        singleType = 'Float';
      }
